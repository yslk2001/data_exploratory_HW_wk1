
#plot3

#install.packages("lubridate")
library(lubridate)

d <- read.table(file="C:/../exdata_data_household_power_consumption/household_power_consumption.txt",sep= ";", header=TRUE)

head(d)
d$Date <- as.Date(d$Date, format = "%d/%m/%Y")
d_new <- d[d$Date >= "2007-02-01" & d$Date <= "2007-02-02", ]
dim(d_new)
d_new
table (d_new$Date)
table (d_new$Global_active_power)



class(d_new$Global_active_power)
d_new$Global_active_power1 <- as.numeric(as.factor(d_new$Global_active_power))
summary(d_new$Global_active_power1)


summary(d_new$Global_active_power1)


d_new$time1 <- paste(d_new$Date, d_new$Time)


d_new$newtime <- ymd_hms(d_new$time)

 

png("plot3.png",   width = 480, height = 480, pointsize=12)

with (d_new,  plot(newtime, Sub_metering_1, main = " ", type = "n", ylab="Energy sub metering" , xlab= " " ) )
axis(1, at=c(d_new$newtime[1], d_new$newtime[1441], d_new$newtime[2880]),labels=c("Thu", "Fri", "Sat") )

with(d_new,  points( newtime, Sub_metering_1, col="black", lwd=1 , type="l"  ))

with(d_new,  points( newtime, Sub_metering_2, col="red", lwd=1 , type="l" ) )  

with(d_new,  points( newtime, Sub_metering_3, col="blue", lwd=1 , type="l" ) )

legend ("topright", lty = 1, col = c("black", "red", "blue"), legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))

dev.off()




